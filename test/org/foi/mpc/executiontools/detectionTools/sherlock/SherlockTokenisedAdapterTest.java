...
public class SherlockTokenisedAdapterTest extends AdaptersTestTemplate {

    @Before
    @Override
    public void setUp() throws IOException {
        super.setUp();
        adapter = (new SimilarityDetectionToolFactory(MPCContext.MATCHES_DIR)).createTool(SherlockTokenisedAdapter.NAME);
    }
    ...
    public class withDummySourceDirAndNoRealRun extends AdaptersTestTemplate.withDummySourceDirAndNoRealRun {
        ...
        public void setUp() throws IOException {
            super.setUp();
            adapter = new SherlockTokenisedAdapterStub();
        }
        ...
        private void defultGlobalSettingForDetectionAreOk() {
            ...
            assertFileTypeIsEnabled(SherlockFileType.TOKENISED);
            assertFileTypeIsDisabled(SherlockFileType.ORIGINAL);
        }
        ...
        private void defaultProfileSettingForDetectionAreOK() {
            FileTypeProfile profile = getProfileSettings(SherlockFileType.TOKENISED);
            ...
            assertEquals(profile.getMinStringLength(), 5);
            assertEquals(profile.getMaxJumpDiff(), 1);
        ...
        public void isExecutionPhaseAndHasCorrectGetName() {
            ...
            assertEquals(SherlockTokenisedAdapter.NAME,adapter.getName());
        }
    }
    ...
    public class withValidIdenticalFiles extends AdaptersTestTemplate.withValidIdenticalFiles {
        ...
        public void preprocesIsRun() throws IOException {
            File originalDir = new File(sourceDir.getPath() + File.separator + "tokenised");
            SherlockAdapter sa = (SherlockAdapter) adapter;
        ...
        public void sherlockDetectionWithPreprocessWorks() throws IOException {
            ...
            assertPreprocessingWorks(sourceDir, "tokenised");
        ...
        protected void assertEndLinesOf100PrecentMatchPart(List<MPCMatchPart> matchPart) {
            assertEquals(18, matchPart.get(0).endLineNumberA);
            assertEquals(18, matchPart.get(0).endLineNumberB);
        }
    ...
    public class withValidNonIdenticalFiles extends AdaptersTestTemplate.withValidNonIdenticalFiles {
        ...
        protected void assertMatchDataIsOkFileContaingingFileTwice(File matchFile, MPCMatch match) {
            ...
            System.out.println("C: "+match.calculatedSimilarity+" "+match.calculatedSimilarityA+" "+match.calculatedSimilarityB);
            ...
            assertEquals(127, match.calculatedSimilarity, 0.0001);
            assertEquals(127, match.calculatedSimilarityA, 0.0001);
            assertEquals(127, match.calculatedSimilarityB, 0.0001);
        ...
        protected void assertMatchDataIsOk(File matchFile, MPCMatch match) {
            assertEquals(matchFile.getName().split("-")[0], match.fileAName);
            assertEquals(matchFile.getName().split("-")[1], match.fileBName);
            assertEquals(84, match.similarity, 0.0001);
            assertEquals(84, match.similarityA, 0.0001);
            assertEquals(84, match.similarityB, 0.0001);
            assertEquals(2, match.matchParts.size());
        ...
        protected void assertFirstMatchPartIsOk(List<MPCMatchPart> matchPart) {
            ...
            assertEquals(5, matchPart.get(0).endLineNumberA);
            assertEquals(5, matchPart.get(0).endLineNumberB);
            assertEquals(23, matchPart.get(0).similarity, 0.0001);
            assertEquals(23, matchPart.get(0).similarityA, 0.0001);
            assertEquals(23, matchPart.get(0).similarityB, 0.0001);
        }

        @Override
        protected void assertSecondMatchPartIsOk(List<MPCMatchPart> matchPart) {
            assertEquals(12, matchPart.get(1).startLineNumberA);
            assertEquals(6, matchPart.get(1).startLineNumberB);
            assertEquals(24, matchPart.get(1).endLineNumberA);
            assertEquals(18, matchPart.get(1).endLineNumberB);
            assertEquals(61, matchPart.get(1).similarity, 0.0001);
            assertEquals(61, matchPart.get(1).similarityA, 0.0001);
            assertEquals(61, matchPart.get(1).similarityB, 0.0001);
        }
    ...
    private class SherlockTokenisedAdapterStub extends SherlockTokenisedAdapter {
